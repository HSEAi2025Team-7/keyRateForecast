...
# разовая инициализация пакетов для скорости работы
...
data = pd.read_csv('cbr_key-rate_press_releases.csv')

def clean(text):
    """
    очистка текста от знаков пробела и запрета переноса,
...
def sentence(text):
    '''
    переводим текст в список строк по предложениям
    '''
...
def stopword(sentences):
    """
    удаляем стоп слова в тексте, знаки препинания, числа
    ...
    for i in sentences:
        for w in word_tokenize(i, language='russian'):
            ...
            if not w:
                continue
            if w in punct or w in punct_add: #убираем знаки пунктуации
                continue
            if re.fullmatch(r'\d+([.,]\d+)?', w):  #убираем числовые токены
                continue
            if re.fullmatch(r'\d{1,2}:\d{2}(:\d{2})?', w): # убираем время 
                continue
            if re.fullmatch(r'\d{1,2}\.\d{1,2}\.\d{2,4}', w): # убираем даты
                continue
            if re.fullmatch(r'\d{4}-\d{4}', w): # убираем год-год
                continue
            if re.fullmatch(r'\d+(?:[.\-\/]\d+)+', w): # убираем число с разделителями
                continue
            if re.fullmatch(r'\d+(?:[.,]\d+)?\s*-\s*\d+(?:[.,]\d+)?', w): #убираем числовые значения с тире
                continue
            if w in ru_stop: #убираем стоп-слова
                continue
...
def key_rate(text):
    '''
    Функция возвращает размер ключевой ставки
    '''
    if not isinstance(text, str) or not text.strip():
        return None
    t = text.lower()
...
def process_series(text_sr: pd.Series, title_sr: pd.Series = None):
    """
    делает словарь промежуточных результатов из функци выше
    """
    ...
    tokens = sents.map(stopword).map(abb) #применяем функцию очистки от стоп слов к разделенному по предложениям тексту, расшифровки аббревитарур 
